<x-layouts.app :title="__('Edit Tag')">
    <div class="flex h-full w-full flex-1 flex-col gap-6">
        <div>
            <x-heading size="xl" level="1">{{ __('Edit Tag') }}</x-heading>
        </div>

        <div class="max-w-lg">
            <x-form method="put" action="{{ route('tags.update', $tag) }}" class="space-y-4">
                <x-input type="text" name="name" :label="__('Tag Name')" :value="old('name', $tag->name)" required />

                <div>
                    <x-label for="color" :value="__('Color')" />
                    <div class="flex items-center gap-3 mt-1">
                        <input type="color" name="color" id="color" value="{{ old('color', $tag->color) }}" class="h-10 w-14 rounded cursor-pointer border border-gray-200 dark:border-gray-700">
