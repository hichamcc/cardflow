<x-layouts.app :title="__('New Event')">
    <div class="flex h-full w-full flex-1 flex-col gap-6">
        ...
        <div>
            <x-heading size="xl" level="1">{{ __('New Event') }}</x-heading>
        </div>
        ...
        <x-form method="post" action="{{ route('events.store') }}">
            <div class="mx-auto max-w-2xl space-y-6">
                <div class="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
                    <div class="space-y-4">
                        ...
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div>
                                ...
                                <select id="type" name="type"
                                    ...
                                    <option value="call" {{ old('type') === 'call' ? 'selected' : '' }}>{{ __('Call') }}</option>
                                    <option value="task" {{ old('type') === 'task' ? 'selected' : '' }}>{{ __('Task') }}</option>
                                    <option value="reminder" {{ old('type') === 'reminder' ? 'selected' : '' }}>{{ __('Reminder') }}</option>
                        ...
                        <div>
                            ...
                            <input type="text" id="location" name="location" value="{{ old('location') }}" placeholder="{{ __('Where?') }}"
                                class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-800 dark:text-white dark:placeholder-gray-500">
                        </div>
                        <div>
                            <label for="description" class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Description') }}</label>
                            <textarea id="description" name="description" rows="4" placeholder="{{ __('Event details...') }}"
                                class="w-full rounded-lg border border-gray-300 bg-white px-4 py-3 text-sm text-gray-900 placeholder-gray-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-800 dark:text-white dark:placeholder-gray-500">{{ old('description') }}</textarea>
                        </div>
                        <div>
                ...
                <div class="flex items-center justify-end gap-3">
                    <a href="{{ route('calendar.index') }}" class="rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700">
                        {{ __('Cancel') }}
                    </a>
